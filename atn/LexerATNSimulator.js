...
let LexerATNSimulator = class LexerATNSimulator extends ATNSimulator {
        ...
        for (let c of closure) {
            ...
            for (let ti = 0; ti < n; ti++) { // for each optimized transition
                ...
                if (target != null) {
                    ...
                    let treatEofAsEpsilon = t === IntStream.EOF;
    ...
    computeStartState(input, p) {
        let initialContext = PredictionContext.EMPTY_FULL;
        let configs = new OrderedATNConfigSet();
        for (let i = 0; i < p.numberOfTransitions; i++) {
            let target = p.transition(i).target;
            let c = ATNConfig.create(target, i + 1, initialContext);
        ...
        switch (t.serializationType) {
            ...
            case 6 /* TransitionType.ACTION */:
                if (config.context.hasEmpty) {
                    ...
                    // the split operation.
                    let lexerActionExecutor = LexerActionExecutor.append(config.lexerActionExecutor, this.atn.lexerActions[t.actionIndex]);
    ...
    addDFAState(configs) {
        ...
        let proposed = new DFAState(configs);
        let existing = this.atn.modeToDFA[this.mode].states.get(proposed);
        if (existing != null) {
            return existing;
        }
        configs.optimizeConfigs(this);
        let newState = new DFAState(configs.clone(true));
